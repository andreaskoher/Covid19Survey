...
@add_arg_table! argtable begin
    ...
    "--predictors", "-p"
        ...
        arg_type = String
        default = "total-above18,google,apple,telco"
    "--foldername", "-f"
        arg_type = String
        default = "$(today())"
    "--thinning", "-t"
...
## ==========================================================================
# load data
data = CSV.read(projectdir("data/data_with_predictors.csv"), DataFrame)
kwargs = BSON.load(projectdir("data/data_with_predictors.bson"))

## ==========================================================================
@info "initialize model"

predictors = split(parsed_args["predictors"], ",")
@info predictors
tdata = Covid19Survey.turingformat(data; predictors, kwargs...)
model = try
